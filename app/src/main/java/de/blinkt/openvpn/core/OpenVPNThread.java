...
public class OpenVPNThread implements Runnable {
    ...
    public void run() {
        try {
            Timber.i("Starting openvpn");
            startOpenVPNThreadArgs(mArgv);
            Timber.i("OpenVPN process exited");
        } catch (Exception e) {
            VpnStatus.logException("Starting OpenVPN Thread", e);
            EventBus.getDefault().post(new VpnErrorEvent(e.getMessage()));
            Timber.e("OpenVPNThread Got %s", e.toString());
        } finally {
            int exitValue = 0;
            try {
                if (mProcess != null)
