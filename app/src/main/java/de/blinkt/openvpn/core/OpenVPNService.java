...
public class OpenVPNService extends VpnService implements StateListener, Callback, ByteCountListener {
    ...
    private static String state = "", upload, download;
    private static long byteIn, byteOut;
    boolean flag = false;
    ...
    private void showNotification(final String msg, String tickerText, @NonNull String channel,
        ...
        Timber.i("showNotification: %s", msg);
        ...
        if (Feature.FEATURE_SHOW_ACTION_FROM_NOTIFICATION) {
        ...
        try {
            ...
            Timber.e(th, "Error when show notification");
        }
...
//        TotalTraffic.calcTraffic(this, in, out, diffIn, diffOut);
        if (mDisplayBytecount) {
            ...
            String download = humanReadableByteCount(in, false, getResources());
            String upload = humanReadableByteCount(out, false, getResources());
            String byteIn = String.format("↓%2$s", getString(R.string.statusline_bytecount),
                    download) + " - " + humanReadableByteCount(diffIn / OpenVPNManagement.mBytecountInterval, false, getResources()) + "/s";
            String byteOut = String.format("↑%2$s", getString(R.string.statusline_bytecount),
                    upload) + " - " + humanReadableByteCount(diffOut / OpenVPNManagement.mBytecountInterval, false, getResources()) + "/s";
            ...
            OpenVPNService.upload = upload;
            OpenVPNService.download = download;

            OpenVPNService.byteIn = in;
            OpenVPNService.byteOut = out;
            sendMessage(duration, String.valueOf(lastPacketReceive), byteIn, byteOut, upload, download);
        }
    ...
    //sending message to main activity
    private void sendMessage(String duration, String lastPacketReceive, String byteIn, String byteOut, String upload, String download) {
        Intent intent = new Intent("connectionState");
    ...
    public static long getByteIn() {
        return byteIn;
    }

    public static long getByteOut() {
        return byteOut;
    }
